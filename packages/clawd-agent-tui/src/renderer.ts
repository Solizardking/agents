...
const DEFAULT_FORMATTERS: Record<string, ToolFormatter> = {
  ...
  zk_compute_nullifier: (_n, a) => `context=${trunc(String(a.context ?? ''))}`,
  zk_load_proof: (_n, a) => `path=${trunc(String(a.path ?? ''))}`,
  zk_verify_proof_shape: (_n, a) => `path=${trunc(String(a.proofPath ?? ''))}`,
  zk_route_intent: (_n, a) => `text=${trunc(String(a.text ?? ''))}`,
...
const TOOL_LABELS: Record<string, { past: string; noun: string }> = {
  ...
  zk_compute_nullifier: { past: 'Derived', noun: 'nullifier' },
  zk_load_proof: { past: 'Loaded', noun: 'proof' },
  zk_verify_proof_shape: { past: 'Verified', noun: 'proof shape' },
  zk_route_intent: { past: 'Routed', noun: 'intent' },
...
export class TuiRenderer {
  ...
  constructor(opts: RendererOptions) {
    ...
    this.toolColors = {
      shell: RED,
      web_search: MAGENTA,
      zk_compute_nullifier: CYAN,
      zk_oneshot: CYAN,
      zk_verify_proof_shape: CYAN,
      zk_load_proof: CYAN,
      zk_route_intent: MAGENTA,
      ...opts.toolColors,
    };
  }
