...
export async function registerRoutes(app: Express, existingServer?: Server): Promise<Server> {
  ...
  const isPublicMetaplexAgentPath = (path: string, method: string) => {
    ...
    if ((path === "/api/metaplex-agents/mint" || path === "/api/metaplex-agents/agent" || path === "/api/mint/mint" || path === "/api/mint/agent") && method === "POST") return true;
    if ((path === "/api/metaplex-agents/register" || path === "/api/mint/register") && method === "POST") return true;
    if ((path.startsWith("/api/metaplex-agents/fetch/") || path.startsWith("/api/mint/fetch/")) && method === "GET") return true;
    return false;
  };
  ...
  const isPublicBrowserAgentsPath = (path: string, method: string) => {
    ...
    return (
      ...
      /^\/api\/clawd\/browser-agents\/[^/]+\/(adapter-status|bridge|operational-data)$/.test(path) ||
  ...
  const isPublicCloudflareStreamPath = (path: string, method: string) => {
    ...
    if (method === "GET" && /^\/api\/cloudflare-stream\/(playback|health|views)\/[^/]+$/.test(path)) {
      return true;
    }
    return false;
  };
  ...
  const isLiveTradingPath = (path: string, method: string) => {
    ...
    if (/^\/api\/pump\/(metadata|launch|build-buy|build-sell|submit)$/.test(path) && method === "POST") return true;
    if (/^\/api\/tokens\/[^/]+\/(buy|sell)$/.test(path) && method === "POST") return true;
    if (path === "/api/tokens/execute" && method === "POST") return true;
    if (/^\/api\/dbc\/(create-config|launch|build-swap|submit|migrate)$/.test(path) && method === "POST") return true;
    if (path === "/api/flash/quote" && method === "POST") return true;
    if (/^\/api\/meteora-swap\/(build-swap|build-add-liquidity|submit|create-pool|seed-liquidity|add-liquidity)$/.test(path) && method === "POST") return true;
    if (/^\/api\/wallet-ops\/(jupiter-order|jupiter-build)$/.test(path) && method === "GET") return true;
    ...
    if (/^\/api\/jupiter-ultra\/trigger\/(execute-order|cancel-order)$/.test(path) && method === "POST") return true;
    if (path === "/api/dflow/order" && method === "GET") return true;
    if (/^\/api\/dflow\/(swap|swap-instructions|submit-intent)$/.test(path) && method === "POST") return true;
    return false;
  };
  ...
  const requireCurrentHolderAccess = async (
    ...
    if (principal?.type === "api-key") {
      ...
      const access = await getWalletAccessState(principal.walletAddress, { failOpenOnRpcError: false });
      if (!access.allowed) {
  ...
  app.use((req, res, next) => {
    void (async () => {
      ...
      if (isLivekitManagementPath(req.path, req.method)) return requireApiAdminOrSession(req, res, next);
      if (isPublicLivekitPath(req.path, req.method)) return next();
      if (isCloudflareStreamManagementPath(req.path, req.method)) return requireApiAdminOrSession(req, res, next);
      ...
      if (/^\/api\/backpack\/(order|orders|cancel|cancel-all)(\/|$)/.test(req.path) && req.method === "POST") {
        return requireApiAdminOrSession(req, res, next);
      }
      if (isPublicStocksPath(req.path, req.method)) return next();
      if (isLiveTradingPath(req.path, req.method)) return requireCurrentHolderAccess(req, res, next, "trading");
      if (isPaidAiProviderPath(req.path, req.method)) return requireCurrentHolderAccess(req, res, next, "paid_ai");
      if (isPaidRpcOrDataPath(req.path, req.method)) return requireCurrentHolderAccess(req, res, next, "paid_data");
